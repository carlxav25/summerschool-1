program basic
  use mpi
  use iso_fortran_env, only : REAL64

  implicit none
  integer, parameter :: size = 10000000
  integer :: rc, myid, ntasks
  integer :: message(size)
  integer :: receiveBuffer(size)
  integer :: status(MPI_STATUS_SIZE,2)
  integer :: source, dest
  integer :: request(2)
  real(REAL64) :: t0, t1

  call mpi_init(rc)
  call mpi_comm_rank(MPI_COMM_WORLD, myid, rc)
  call mpi_comm_size(MPI_COMM_WORLD, ntasks, rc)

  message = myid

  ! Start measuring the time spent in communication
  call mpi_barrier(mpi_comm_world, rc)
  t0 = mpi_wtime()

  ! TODO: Send and receive as defined in the assignment

  if (myid < ntasks-1) then
     dest = myid + 1
  else
     dest = MPI_PROC_NULL
  end if

  if (myid > 0) then
     source = myid - 1
  else
     source = MPI_PROC_NULL
  end if


 ! if ( myid .gt. 0 .and. myid .lt. ntasks-1 ) then
!  if (myid.lt.ntasks) then 
! call mpi_sendrecv(message,size,mpi_integer,dest,myid+1, &
!                    receivebuffer,size, mpi_integer, &
!                    source, myid, mpi_comm_world,status,rc)
 
 ! call mpi_irecv(receiveBuffer, size, MPI_INTEGER, source, &
 !                        MPI_ANY_TAG, MPI_COMM_WORLD, request(2), rc)
  ! Send messages in the back ground
!  call mpi_isend(message, size, MPI_INTEGER, dest, &
!       myid + 1, MPI_COMM_WORLD, request(1), rc)
 
 call mpi_recv_init(receiveBuffer, size, MPI_INTEGER, source, &
                         MPI_ANY_TAG, MPI_COMM_WORLD, request(2), rc)
  ! Send messages in the back ground
  call mpi_send_init(message, size, MPI_INTEGER, dest, &
       myid + 1, MPI_COMM_WORLD, request(1), rc)
  call mpi_startall(2, request,rc)
  ! Blocking wait for messages
  call mpi_waitall(2, request, status, rc)
     write(*,'(A10,I3,A20,I8,A,I3,A,I3)') 'Sender: ', myid, &
          ' Sent elements: ',size, &
          '. Tag: ', myid+1, '. Receiver: ', myid+1

     write(*,'(A10,I3,A,I3)') 'Receiver: ', myid, &
          ' First element: ', receiveBuffer(1)
    !  write(*,*) 'Status', status(mpi_source)
  

  ! Finalize measuring the time and print it out
  t1 = mpi_wtime()
  call mpi_barrier(mpi_comm_world, rc)
  call flush(6)

  write(*, '(A20, I3, A, F6.3)') 'Time elapsed in rank', myid, ':', t1-t0

  call mpi_finalize(rc)

end program basic
